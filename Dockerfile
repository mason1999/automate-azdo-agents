################################################################################
# Install minimum tooling
################################################################################
FROM ubuntu:latest
RUN apt-get update && apt-get install -y curl git iputils-ping tmux jq xclip expect
ENV DISPLAY=:0
...
RUN cat <<'EOF' > ${HOME}/.config/nvim/colors/minimalist.vim
set background=dark
if version > 580
    hi clear
    if exists("syntax_on")
...
# Use <C-a>r to reload the rmux configuration file
unbind r
...
################################################################################
WORKDIR /azdo-agent
COPY ./add-config-automated.sh .
COPY ./remove-config-automated.sh .

################################################################################
# Azdo required build variables
################################################################################
ARG ORG_NAME
ARG CLIENT_ID
ARG CLIENT_SECRET
...
ARG AGENT_POOL_NAME
ARG AGENT_NAME
ARG AGENT_WORKING_DIRECTORY

################################################################################
# Azdo environment variables (build and runtime)
################################################################################
ENV ORG_NAME=$ORG_NAME
ENV CLIENT_ID=$CLIENT_ID
ENV CLIENT_SECRET=$CLIENT_SECRET
ENV TENANT_ID=$TENANT_ID
ENV AGENT_POOL_NAME=$AGENT_POOL_NAME
ENV AGENT_NAME=$AGENT_NAME
ENV AGENT_WORKING_DIRECTORY=$AGENT_WORKING_DIRECTORY

################################################################################
# Required for build when running automated config
################################################################################
ENV AGENT_ALLOW_RUNASROOT=true

RUN curl -sSL "https://vstsagentpackage.azureedge.net/agent/4.248.1/vsts-agent-linux-x64-4.248.1.tar.gz" -o azdo-agent.tar.gz \
    && tar -xzvf azdo-agent.tar.gz \
    && rm azdo-agent.tar.gz \
    && ./bin/installdependencies.sh \
    && chmod a+x ./add-config-automated.sh \
    && ./add-config-automated.sh -n "${AGENT_NAME}" -w "${AGENT_WORKING_DIRECTORY}"

ENTRYPOINT ["bash", "-c", "./run.sh & sleep infinity"]
